...
def handle_event(id: str, details: dict):
    ...
    try:
        ...
        if details['source'] == 'ADP':
            ...
            response = http.request("POST", "http://scada:6069/data", body=encoded_data, headers={'Content-Type': 'application/json'})
            # data = json.loads(resp.data.decode('utf-8'))['json']
            if status == "warning" or status == "fatal":
                payload = {"status": status}
                encoded_data = json.dumps(payload).encode('utf-8')
                response = http.request("POST", "http://protection:6068/alarm", body=encoded_data, headers={'Content-Type': 'application/json'})
            ...
            if (t == "protection"):
                ...
                encoded_data = json.dumps(payload).encode('utf-8')
                response = http.request("POST", "http://protection:6068/check", body=encoded_data, headers={'Content-Type': 'application/json'})
            else:
                payload = {"value": current_value, "key": value}
                encoded_data = json.dumps(payload).encode('utf-8')
                response = http.request("POST", "http://scada:6069/check", body=encoded_data, headers={'Content-Type': 'application/json'})
...
def consumer_job(args, config):
    ...
    # Subscribe to topic
    topic = TOPIC_NAME
    transmitter_consumer.subscribe([topic, 'transmitter1'], on_assign=reset_offset)
